...
import { RootState, setUser } from '../../index';
    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex justify-between items-end mb-12">
        <div>
          ...
          <p className="text-gray-500">Manage your administrative details and account security.</p>
        </div>
        <button 
          onClick={() => setIsEditing(!isEditing)}
          className={`px-6 py-2.5 rounded-lg text-sm font-black transition-all ${isEditing ? 'bg-red-50 text-red-500' : 'bg-primary text-accent shadow-lg shadow-primary/20'}`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-1 space-y-6">
          <div className="bg-white border border-gray-200 rounded-3xl p-8 flex flex-col items-center text-center shadow-sm">
            ...
            <p className="text-accent font-black uppercase text-[10px] tracking-widest mt-1 bg-accent/10 px-3 py-1 rounded-full">{user?.role}</p>
        ...
        <div className="md:col-span-2">
          <form onSubmit={handleUpdate} className="bg-white border border-gray-200 rounded-3xl p-8 shadow-sm h-full">
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="text-xs font-black text-gray-400 uppercase tracking-widest block mb-2">Administrative Role</label>
                <select 
                  disabled={!isEditing}
                  className="w-full bg-slate-50 border-none rounded-xl p-4 text-sm focus:ring-1 ring-primary font-bold disabled:opacity-60"
                  value={tempUser?.role || ''}
              ...
              {isEditing && (
                <div className="pt-8 flex justify-end">
                  <button type="submit" className="bg-primary text-accent px-8 py-3 rounded-xl font-black shadow-xl shadow-primary/20 hover:brightness-110 transition-all">
                    Update Profile Data
                  </button>
                </div>
              )}
